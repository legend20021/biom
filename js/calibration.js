...
// Variable para controlar el estado de calibración de pH
let isPhCalibrationInProgress = false;
...
async function calibratePH(nivel) {
    ...
    if(res) {
        ...
        // Obtener todos los botones de pH
        const ph4Btn = document.getElementById('ph4-btn');
        const ph7Btn = document.getElementById('ph7-btn');
        const ph10Btn = document.getElementById('ph10-btn');

        // Deshabilitar todos los botones de pH
        ph4Btn.disabled = true;
        ph7Btn.disabled = true;
        ph10Btn.disabled = true;
        ...
        setTimeout(() => {
            // Marcar que la calibración terminó
            isPhCalibrationInProgress = false;
            
            // Rehabilitar todos los botones de pH
            ph4Btn.disabled = false;
            ph7Btn.disabled = false;
            ph10Btn.disabled = false;
            
            // Remover clase de éxito del botón actual
            button.classList.remove('success');
            progress.style.width = '0%';
        }, 2000);
    }
